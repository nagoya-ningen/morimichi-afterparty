...
- 投稿項目：名前（任意・ニックネーム可）／参加した曜日（複数選択可）／SNS URL（任意）／感想本文（5字以上）／感想の対象（アーティスト・出店・エリア・森道市場・運営への感謝）／写真（任意・1枚）
- **事後モデレーション制**：すべての投稿は即時公開され、不適切な投稿は通報経由で運営が即時非表示にする
...
- **管理ページ** `admin.html`：運営者だけが使う。メールアドレスとパスワードでログインし、通報対応・非表示などのモデレーションを行う
...
- 不適切な写真は通報経由で運営が即時非表示にする（事後モデレーション）
