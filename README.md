...
- 投稿項目：名前（必須・ニックネーム可）／行った曜日／Instagram（任意）／感想本文（30字以上）／写真（任意）／感想の対象（アーティスト・店舗・ショップ・エリア・森道自体・運営への感謝）
- 誹謗中傷フィルタ（NGワード辞書）＋通報＋運営による非表示
- 写真は投稿前に位置情報などのExifを自動除去
- バックエンドは Firebase（Firestore + Storage + 匿名認証 + App Check）
...
このアプリは投稿を保存するために Firebase を使います。以下を1ステップずつ進めてください。**無料の Spark プランで動きます。**

### 1. Firebase プロジェクトを作成

1. <https://console.firebase.google.com/> にアクセス（Googleアカウントでログイン）
2. 「プロジェクトを追加」→ 任意の名前（例：`morimichi-afterparty`）
...
2. アプリのニックネームを入力して登録
3. 表示される `firebaseConfig`（apiKey 等）をコピー
...
2. 「Sign-in method」タブ →「匿名」を**有効**にする

### 6. App Check（reCAPTCHA v3）を設定
...
3. Firebase コンソール「App Check」→ アプリを登録し、プロバイダに reCAPTCHA v3、上記の **シークレットキー** を登録
4. App Check で **Firestore** と **Storage** を「適用（Enforce）」にする

### 7. セキュリティルールを反映

1. Firestore コンソール →「ルール」タブ → `firestore.rules` の中身を貼り付けて「公開」
2. Storage コンソール →「ルール」タブ → `storage.rules` の中身を貼り付けて「公開」
...
Firebase の承認済みドメインに `localhost` を入れておくこと（手順8）。
...
- 写真の Exif（GPS位置情報・撮影日時・端末モデル）は投稿前に Canvas 再エンコードで完全除去
- 投稿に含まれるのは、投稿者が自分で入力した情報のみ
